...
```{python}
alt.Chart(df).mark_bar().encode(
  alt.X("min(precip)", axis = alt.Axis(title = "Minimum Rain")),
  alt.Y("city", axis = alt.Axis(title = "City"))
)
```
...
Memo: You can choose 20 kinds of "Mark"s: A grammatical rule!
...
Memo: Code writen in Python - Altair -> JSON - Vega -> Visual

Memo: Can see the JSON script by the method .to_json()
...
Memo: Can load as a JSON script by json.loads() -> Then can order despcriptively (to extract relevant subsets, e.g.)
